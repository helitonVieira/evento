Public Class ClIngresso
    ...
    Public Sub CadastrarIngresso()
        sql = "Insert Into tab_ingresso( cod_evento,des_ingresso,des_ingresso1, val_entrada, des_lote, des_observacao1, des_observacao2,ind_aberto) Values ('" & cod_evento & "' ,'" & des_ingresso & "' ,'" & des_ingresso1 & "' , '" & val_entrada & "','" & des_lote & "' ,'" & des_observacao1 & "' ,'" & des_observacao2 & "', '" & ind_aberto & "')"
        con.Operar(sql)
    End Sub
    ...
    Public Function UltimoEventoCadastrado()
        sql = "Select max(cod_evento) + 1 From tab_evento"
        ds = con.Listar(sql)
        Return ds
    End Function
End Class
